...
def ProcessClothingJSON(input_path: Path, output_path: Path, gcAgeInfo: plAgeInfo, singlePage: Union[None, str] = None) -> Generator[plLocation]:
    ...
    with input_path.open("r") as clothingFile:
        ...
        for pageInfo in pages:
            if singlePage and singlePage != pageInfo['name']:
                continue

            print(f"Creating {pageInfo['name']}...")
            yield CreatePage(input_path.parent, output_path, gcAgeInfo, pageInfo)

def main(input_path: Path, output_path: Path, round_trip: bool, single_page: Union[None, str] = None) -> None:
    version = pvMoul
